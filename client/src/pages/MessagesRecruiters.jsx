...
const ContextMenu = ({ x, y, msg, isMe, onClose, onReply, onForward, onInfo, onDeleteForMe, onDeleteForEveryone, onSelect }) => {
  ...
  return (
    <div ref={ref} style={{ position: 'fixed', zIndex: 1000, top: y, left: x, background: '#233138', border: '1px solid #2a3942' }}
  ...
  return (
    <div ref={ref} className="absolute bottom-14 left-0 rounded-2xl shadow-2xl p-3 z-50 w-72"
      ...
      <div className="grid grid-cols-10 gap-1">
        {EMOJIS.map(e => <button key={e} onClick={() => onSelect(e)} className="text-xl hover:bg-[#2a3942] rounded p-1 transition-colors">{e}</button>)}
      </div>
    </div>
  );
};
...
const InfoModal = ({ msg, onClose, resolveName }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
    <div className="rounded-2xl shadow-2xl p-6 min-w-[320px] max-w-md" style={{ background: '#233138' }} onClick={e => e.stopPropagation()}>
      ...
      <div className="mt-5 p-3 rounded-xl" style={{ background: '#2a3942' }}><p className="text-white text-sm">{msg.content}</p></div>
    </div>
  </div>
);
...
export default function MessagesRecruiters() {
  ...
  }, [messages, myId, managers, recruiters, readSet]);
                          ...
                          {managers.filter(m => (buildName(m)||'').toLowerCase().includes(recipientSearch.toLowerCase())).map(m => {
                            ...
                            return (
                              <button key={id} onClick={() => { if (composeRecipients.includes('all')) return; setComposeRecipients(prev => checked ? prev.filter(r => r !== id) : [...prev, id]); }}
                                className={`flex items-center gap-3 w-full px-4 py-2.5 transition-colors text-left ${checked ? 'bg-[#00a884]/20' : 'hover:bg-[#2a3942]'} ${composeRecipients.includes('all') ? 'opacity-40 cursor-not-allowed' : ''}`}>
