...
const ContextMenu = ({ x, y, msg, isMe, onClose, onReply, onForward, onInfo, onDeleteForMe, onDeleteForEveryone, onSelect }) => {
  ...
  return (
    <div ref={menuRef} style={{ position: 'fixed', zIndex: 1000, top: y, left: x, background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '12px', boxShadow: '0 8px 32px rgba(0,0,0,0.12)' }}
  ...
  return (
    <div ref={ref} className="absolute bottom-14 left-0 mb-2 rounded-2xl shadow-2xl p-3 z-50 w-72"
      ...
      <div className="grid grid-cols-10 gap-1">
        {EMOJIS.map(e => (
          <button key={e} onClick={() => onSelect(e)} className="text-xl hover:bg-gray-100 rounded p-1 transition-colors">{e}</button>
        ))}
      </div>
    </div>
  );
...
const InfoModal = ({ msg, onClose, resolveName }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
    <div className="rounded-2xl shadow-2xl p-6 min-w-[320px] max-w-md" style={{ background: "#ffffff" }} onClick={e => e.stopPropagation()}>
      ...
      <div className="mt-5 p-3 rounded-xl" style={{ background: '#e9edef' }}>
        ...
        {showCompose ? (
                          ...
                          {managers.filter(m => (buildName(m)||'').toLowerCase().includes(recipientSearch.toLowerCase()) && (m._id||m.id) !== myId).map(m => {
                            ...
                            return (
                              <button key={id} onClick={() => { if (composeRecipients.includes('all')) return; setComposeRecipients(prev => checked ? prev.filter(r => r !== id) : [...prev, id]); }}
                                className={`flex items-center gap-3 w-full px-4 py-2.5 transition-colors text-left ${checked ? 'bg-[#00a884]/20' : 'hover:bg-gray-100'}`}>
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-4 select-none">
            ...
            <div className="text-center">
              <p className="text-gray-800 text-xl font-light mb-1">Manager Messages</p>
